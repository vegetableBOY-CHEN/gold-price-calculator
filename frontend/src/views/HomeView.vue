...
const brandRefreshText = computed(() => {
  const seconds = prices.value?.brand_refresh_interval_seconds ?? 1800
    ...
    <section class="market-grid" aria-label="国际国内大盘价格">
      <article v-for="item in marketPrices" :key="`${item.market}-${item.symbol}`" class="quote-card">
        <div class="quote-head">
          ...
          <span :class="['source-pill', item.status === 'live' ? 'live' : 'fallback']">
            {{ item.status === 'live' ? '实时' : '兜底' }}
          </span>
        </div>
        ...
        <div class="quote-meta">
          <span>{{ item.exchange }} · {{ item.symbol }}</span>
...
.brand-table td:first-child strong,
.brand-table td:first-child span {
  display: block;
}
